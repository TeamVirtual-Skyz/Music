...
async def stop_cmd(_, message): 
    ...
    if not await is_active_chat(chat_id):
        ...
        await message.reply_text(f"**😍 Music Dilanjutkan Oleh {checking}!**")
...
async def stop_cmd(_, message): 
    ...
    if await is_active_chat(chat_id):
        ...
        await remove_active_chat(chat_id)
        await music.pytgcalls.leave_group_call(chat_id)
        await message.reply_text(f"**😏 Music Telah Berakhir/Dihentikan Oleh {checking}!**") 
    else:
        return await message.reply_text("Saya tidak berpikir jika ada sesuatu yang diputar di obrolan suara")
...
<b>⏳ Durasi:</b> {duration}
<b>⚜️ Di play oleh:</b> {semx.mention}
"""
                    ),
                )
                os.remove(thumb)
